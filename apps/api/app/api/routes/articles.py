...
from app.schemas.article_raw import ArticleRawCreate, ArticleRawRead
...
@router.post("/raw", response_model=ArticleRawRead, status_code=status.HTTP_201_CREATED)
def create_article_raw_route(
    payload: ArticleRawCreate, db: Session = Depends(get_db)
) -> ArticleRawRead:
    article_raw = create_article_raw(db=db, payload=payload)
    if not article_raw:
        raise HTTPException(status_code=404, detail="Source not found.")
    return article_raw
